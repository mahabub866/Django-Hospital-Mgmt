...
def Add_Member(request):
    ...
    if not request.user.is_staff:
        return redirect('login')
    member=Member.objects.all()
    plan1=Plan.objects.all()#eita o ansi
    if request.method=="POST":
        n=request.POST['name']
        c = request.POST['contact']
        e = request.POST['emailid']
        ...
        ia = request.POST['initial']

        plan= Plan.objects.filter(name=p).first()
        try:
            Member.objects.create(name=n,contact=c,emailid=e,age=a,gender=g,plan=plan ,joindate=j,expiredate=ex,initial=ia)#plan=plan dropdown work for another pGE
            error="no"
        except:
            error="yes"
    d={'error': error,'plan':plan1}#eitai o kaj hoise
